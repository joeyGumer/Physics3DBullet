...
update_status ModulePhysics3D::PreUpdate(float dt)
{
	...
	int total_manifolds = world->getDispatcher()->getNumManifolds();
	for (int i = 0; i < total_manifolds; i++)
	{
		btPersistentManifold* manifold = world->getDispatcher()->getManifoldByIndexInternal(i);
		PhysBody3D* pbody1 = (PhysBody3D*)manifold->getBody0()->getUserPointer();
		PhysBody3D* pbody2 = (PhysBody3D*)manifold->getBody1()->getUserPointer();
		if (pbody1 && pbody2)
		{
			p2List_item<Module*>* listener = pbody1->collision_listeners.getFirst();
			if (listener)
			{
				listener->data->OnCollision(pbody1, pbody2);
				listener = listener->next;
			}

			listener = pbody2->collision_listeners.getFirst();
			if (listener)
			{
				listener->data->OnCollision(pbody2, pbody1);
				listener = listener->next;
			}
		}
	}
...
bool ModulePhysics3D::CleanUp()
{
	...
	p2List_item<btCollisionShape*>* shape = collision_shapes.getFirst();
	for (; shape; shape = shape->next)
		delete shape->data;

	collision_shapes.clear();

	p2List_item<btRigidBody*>* body = bodies.getFirst();
